...
export default function OptimizedImage({ src, alt, className = '', priority = false, sizes, objectFit = 'contain' }: Props) {
  ...
  return (
    <div className={`relative overflow-hidden ${className}`}>
      ...
      {failed && <div className="absolute inset-0 grid place-items-center text-xs text-muted bg-paper">تعذر تحميل الصورة</div>}
    </div>
  );
}
