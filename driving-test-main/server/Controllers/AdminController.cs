...
public class AdminController : ControllerBase
{
    ...
    {
        if (request.Options.Count != 4 || request.Options.Any(string.IsNullOrWhiteSpace)) return BadRequest(new { message = "يجب أن يحتوي السؤال على 4 اختيارات مكتملة." });
    ...
    public async Task<ActionResult<Question>> UpdateQuestion(int id, QuestionUpsertRequest request)
    {
        ...
        if (request.Options.Count != 4 || request.Options.Any(string.IsNullOrWhiteSpace) || request.CorrectAnswerIndex < 0 || request.CorrectAnswerIndex >= request.Options.Count)
