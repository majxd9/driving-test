...
public class AdminController : ControllerBase
{
    ...
    public async Task<ActionResult<List<StudentResponse>>> GetAllStudents()
    {
        ...
        var stats = await _db.ExamAttempts
            ...
            .Select(g => new { StudentId = g.Key, Attempts = g.Count(), Passed = g.Count(a => a.Correct >= 25) })
        ...
        return Ok(students
            .Select(s =>
            {
                ...
                return ToResponse(s, st?.Attempts ?? 0, st?.Passed ?? 0);
            })
    ...
    public async Task<ActionResult<List<ExamAttemptResponse>>> GetAttempts(string id)
    {
        ...
        return Ok(attempts.Select(a => new ExamAttemptResponse(
            a.Id, a.ModelId, a.Correct, a.Total, a.Answered, a.WrongQuestionIds, a.CreatedAt)).ToList());
    }
    ...
    public async Task<IActionResult> Analytics()
    {
        ...
        var legacyExams = await _db.ExamResults.AsNoTracking().ToListAsync();
        var attemptsData = await _db.ExamAttempts.AsNoTracking().ToListAsync();
        var examTotal = legacyExams.Count + attemptsData.Count;
        var passedTotal = legacyExams.Count(x => x.Passed) + attemptsData.Count(x => x.Correct >= 25);
        var scoreTotal = legacyExams.Sum(x => (double)x.Correct) + attemptsData.Sum(x => (double)x.Correct);
        ...
        return Ok(new {
            ...
            exams = new { total=examTotal, passed=passedTotal, passRate=examTotal==0?0:Math.Round(passedTotal*100.0/examTotal,1), averageScore=examTotal==0?0:Math.Round(scoreTotal/examTotal,1) },
    ...
    private static StudentResponse ToResponse(ApplicationUser u, int attemptCount = 0, int passCount = 0) =>
        new(u.Id, u.UserName ?? "", u.FullName, u.IsActive, !string.IsNullOrEmpty(u.DeviceId),
            u.AccessExpiresAt, u.CreatedAt, attemptCount, passCount);
}
