...
export default function Exam() {
  ...
  if (loading) return (
    <div className="page-shell flex items-center justify-center px-4">
      <div className="surface-panel w-full max-w-xl p-5"><div className="skeleton h-44 rounded-2xl" /></div>
    </div>
  );

  if (loadError || !questions.length) return (
    <div className="page-shell flex items-center justify-center px-5">
      <div className="surface-panel w-full max-w-md text-center p-7">
        <div className="brand-mark mx-auto mb-4">ر</div>
        <h1 className="text-xl font-black mb-2">تعذر تحضير الاختبار</h1>
        <p className="text-muted text-sm leading-relaxed">{loadError ?? 'لم يتم العثور على أسئلة.'}</p>
        <button onClick={loadExam} className="primary-cta mt-5 w-full">إعادة المحاولة</button>
      </div>
    </div>
  );
  ...
  return (
    <div className="exam-page-v2" dir="rtl">
      ...
      <main className="exam-stage-v2">
        <section className="exam-card-v2">
          <div className="exam-card-topline">
            <span>سؤال الاختبار</span>
            <span className="exam-current-badge">{current + 1}</span>
          </div>
          ...
          <div className="exam-question-v2">{q.text}</div>

          <div className="exam-answers-v2">
            {q.options.map((opt, i) => (
              <button
                key={i}
                type="button"
                onClick={() => setAnswers(a => ({ ...a, [q.id]: i }))}
                className={`exam-option-v2 ${answers[q.id] === i ? 'selected' : ''}`}
              >
                <span className="exam-option-letter-v2">{LETTERS[i]}</span>
                <span className="exam-option-text-v2">{opt}</span>
              </button>
            ))}
          </div>
          ...
          <div className="exam-actions-v2">
            <button
              ...
              disabled={current === 0}
              className="exam-action-v2 secondary"
            >السابق</button>
            ...
            <button
              ...
              className="exam-action-v2 next"
            >{isLast ? 'عرض النتيجة' : 'التالي'}</button>
          </div>
        </section>
      </main>

      {imageExpanded && q.imageUrl && (
        <div className="exam-image-modal-v2" onClick={() => setImageExpanded(false)}>
